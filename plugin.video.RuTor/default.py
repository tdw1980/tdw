...
#			xbmcplugin.addDirectoryItem(handle, purl, listitem, True)
			...
			Title = "[B][COLOR FF00FF00][ ПОИСК ][/COLOR][/B]"
			row_url = Title
			listitem = xbmcgui.ListItem(Title)
			listitem.setInfo(type = "Video", infoLabels = {"Title": Title} )
			purl = sys.argv[0] + '?mode=SearchN'\
...
def get_qual(ntor):
	...
	hp=hp.replace("'><img src='/s/i/d.gif' alt='D'",'","')
	hp=hp.replace("'><img src='/s/i/m.png' alt='M'",'')
	hp=hp.replace("</a></td> <td align='right'>",'","')
	hp=hp.replace("<img src='/s/i/com.gif' alt='C' /></td>\n<td align='right'>",'","')
	hp=hp.replace("</td><td align='center'><span class='green'><img src='/s/t/arrowup.gif' alt='S' />&nbsp;",'","')
	hp=hp.replace("</span>&nbsp;<img src='/s/t/arrowdown.gif' alt='L' /><span class='red'>&nbsp;",'","')
