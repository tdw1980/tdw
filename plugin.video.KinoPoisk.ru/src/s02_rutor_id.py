...
def upd(category, sort, text, n):
	...
	if stext=="":
		...
		if text=='1':categoryUrl = httpSiteUrl+'/search/'+n+'/'+category+'/010/'+sort+'/'+stext   #)xt( 0/1/110/0
...
def Storr(category, sort, text, url='0'):
	...
	for tTitle in RootList:
		...
		if len(tTitle)==10 and int(tTitle[8])>0:
			...
			Tresh=["Repack"," PC ","XBOX","RePack","FB2","TXT","DOC"," MP3"," JPG"," PNG"," SCR", "FLAC"]
			for TRi in Tresh:
			...
			if UF==0 and nH<0 and sH<0 and (CT not in TLD):
				...
				Lout.append({"sids":sids,"size":size, "title":xt(Title),"url":row_url})
				#print Title
