...
def GET(target, referer, post_params = None, accept_redirect = True, get_redirect_url = False, siteUrl='www.2KinoPoisk.ru'):
	try:
		...
		sid_file = os.path.join(xbmc.translatePath('special://temp/'), '2kp.cookies.sid')
...
def csr(http):
	...
	for i in LT:
		...
		ss='<td class="size"><span class="badge">'
		es='</span></td>'
		...
		ss='magnet:?xt=urn:btih:'
		es='&dn='
		try: link=mfindal(i, ss, es)[0][len(ss):]
		except: link=""
			
		if link<>"" and isd>0: L.append([seeds, size, title, 'http://torcache.net/torrent/'+link+".torrent"])
	return L
...
class Tracker:
	...
	def Search(self, qury):
		...
		RL=[]
		#for p in range (1,20):
		#	html=get_HTML('http://api.btdigg.org/api/private-c47ba652ee73735a/s02?q='+q+"&p="+str(p))
		#	L=eval(html)
			
			
		#	for i in L:
		#		RL.append([str(i["files"]),str(i["size"]),xt(i["name"]),'http://torcache.net/torrent/'+i["info_hash"]+".torrent"])
				
		html=get_HTML('http://tracker.ru/search?q='+q)
		RL=csr(html)
